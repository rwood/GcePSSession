...
.PARAMETER PowerShellPath
    ...
    If not found at the specified path, the script will search for pwsh.exe in PATH.
...
.NOTES
    ...
    This script can be downloaded and run directly on Windows VMs in Google Cloud.
#>
...
# Convert PowerShell path to 8.3 format for Subsystem line
# Get the short path name (8.3 format)
$shortPath = $PowerShellPath
try {
    $fso = New-Object -ComObject Scripting.FileSystemObject
    $folder = Split-Path $PowerShellPath -Parent
    $file = Split-Path $PowerShellPath -Leaf
    $shortFolder = $fso.GetFolder($folder).ShortPath
    $shortPath = Join-Path $shortFolder $file
    # Convert backslashes to forward slashes for SSH config
    $shortPath = $shortPath.Replace('\', '/')
} catch {
    Write-Warning "Could not get short path, using full path: $_"
    $shortPath = $PowerShellPath.Replace('\', '/')
}
...
$overrideCommentFound = $false

foreach ($line in $lines) {
    $trimmedLine = $line.Trim()
    
    # Check if this is the override comment
    if ($trimmedLine -match '#\s*override\s+default\s+of\s+no\s+subsystems') {
        $overrideCommentFound = $true
        $newLines += $line
        # Add Subsystem line right after the comment
        $subsystemLine = "Subsystem powershell $shortPath -sshs"
        $newLines += $subsystemLine
        $subsystemAdded = $true
        Write-Host "  Added Subsystem line after override comment" -ForegroundColor Gray
        continue
    }
    
    # Remove any existing Subsystem powershell lines (we'll add it in the right place)
    if ($trimmedLine -match '^Subsystem\s+powershell') {
        Write-Host "  Removed existing Subsystem powershell line" -ForegroundColor Gray
        continue
    }
...
# If override comment wasn't found, add it with the Subsystem line
if (-not $overrideCommentFound -and -not $subsystemAdded) {
    # Find a good place to add it (after other Subsystem entries or at end)
    $insertIndex = $newLines.Count
    for ($i = 0; $i -lt $newLines.Count; $i++) {
        if ($newLines[$i] -match '^Subsystem\s+') {
            $insertIndex = $i + 1
            break
        }
    }
    # Insert comment and Subsystem line
    $insertLines = @("# override default of no subsystems", "Subsystem powershell $shortPath -sshs")
    $newLines = $newLines[0..($insertIndex-1)] + $insertLines + $newLines[$insertIndex..($newLines.Count-1)]
    Write-Host "  Added override comment and Subsystem line" -ForegroundColor Gray
}
